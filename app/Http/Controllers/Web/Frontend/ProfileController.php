...
namespace VanguardLTE\Http\Controllers\Web\Frontend {
    ...
    class ProfileController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function postDocument(\Illuminate\Http\Request $request)
        {
            if ($request->license_front && $request->license_back && $request->bill_doc) {
                $request->license_front->move(public_path('user_doc/' . auth()->user()->id), 'license_front');
                $request->license_back->move(public_path('user_doc/' . auth()->user()->id), 'license_back');
                $request->bill_doc->move(public_path('user_doc/' . auth()->user()->id), 'bill_doc');

                auth()->user()->update(['update_license_at' => microtime(true)]);

                return redirect()->back();
            } else {
                return redirect()->back()->withErrors('Please upload all documents');
            }
        }
    }
}
