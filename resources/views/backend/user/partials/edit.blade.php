<div class="box-body box-profile">
    ...
    <div class="form-group">
        <label>@lang('app.phone')@if ($user->phone)
        ...
        <div class="input-group">
            ...
            <input type="text" class="form-control onlynumber" id="phone" name="phone" value="{{ $edit ? $user->phone : '' }}" @if (!auth()->user()->hasRole('admin') &&
                ($user->phone_token || $user->phone)) disabled @endif>
        </div>
    </div>
    ...
    @if ($user->update_license_at > 0)
        <div class="form-group">
            <label>Documents</label>
            <div style="display:flex;">
                <a class="btn btn-default" style="margin-right:10px;" href="/user_doc/{{ $user->id }}/license_front" target="_blank">License front</a>
                <a class="btn btn-default" style="margin-right:10px;" href="/user_doc/{{ $user->id }}/license_back" target="_blank">License back</a>
                <a class="btn btn-default" style="margin-right:auto;" href="/user_doc/{{ $user->id }}/bill_doc" target="_blank">Bill doc</a>

                <form>
                </form>
                @if ($user->is_approved_license == 1)
                    <a class="btn btn-success"disabled style="margin-right:10px;">Approved</a>
                @endif
                @if ($user->is_approved_license != 1)
                    <form id="approve_form" action="{{ route('backend.user.document.approve', ['user' => $user->id]) }}" method="post">
                        @csrf
                ...
                @endif
                @if ($user->is_approved_license != -1)
                    <form id="reject_form" action="{{ route('backend.user.document.reject', ['user' => $user->id]) }}" method="post">
                        @csrf
